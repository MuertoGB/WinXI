...
'  Updated on 15.03.2021 - DS (Added high CPU usage warning)
...
#Region "Load Event Handler"
    ...
    Private Sub FormAssessVerbose_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim Enc As Encoding = CType(IIf(WinSystem.IsWinVista, Encoding.Unicode, Encoding.GetEncoding(Globalization.CultureInfo.CurrentUICulture.TextInfo.OEMCodePage)), Encoding)
...
#Region "AsyncOut"
    ...
    Private Sub AsyncOut(StringAsync As String)
        ...
        If Not StringAsync.Contains("Composition restarted") Then 'Running
            ...
            If WinSystem.IsWinVista() Then 'Vista
...
#Region "Delegate"
    ...
    Private Sub OnExitCode(IntExitCode As Integer)
        ...
        If Not BClosing Then
            ...
            If Not WinsatReader.GetExitCode = -1 Then
                Logger.Log("Exit Code W: " & Int, LogType.WinXILog, RtbLog)
            End If

            Logger.Log("Exit Code X: " & IntExitCode, LogType.WinXILog, RtbLog)
            Logger.Log("Validity Int: " & WinsatApi.GetAssessmentValidityInt(), LogType.WinXILog, RtbLog)
    ...
    Private Sub OnUpdateProgress(IntValue As Integer)
...
#Region "Routines"
    ...
    Private Sub GetInitData()
        ...
        Logger.Log("SystemOS: " & WinSystem.GetWindowsName() & " (Kernel: " & WinSystem.KernelVersion.ProductVersion & ")", LogType.InfoLog, RtbLog)
    ...
    Private Sub KillWinsat()
        If WinsatProc IsNot Nothing Then
            If Not WinsatProc.HasExited Then
                Logger.Log("WinSAT wait...", LogType.WinXILog, RtbLog)
                Do Until WinsatProc.HasExited
                    ...
                    End Try
                Loop
                Logger.Log("WinSAT shutdown complete", LogType.WinXILog, RtbLog)
                Logger.Log("Process finished", LogType.InfoLog, RtbLog)
                Logger.Log("'(ヘ･_･)ヘ┳━┳", LogType.InfoLog, RtbLog)
            End If
        End If
    ...
    Private Sub KillWatcher()
        If Watcher.Enabled Then
            Logger.Log("Stopping ProcessWatcher...", LogType.WinXILog, RtbLog)
...
#Region "Proto"
    ...
    Private Sub ProtoLonghorn(StringAsync As String) 'WINDOWS VISTA
    ...
    End Sub
    Private Sub ProtoOtherOS(StringAsync As String) 'WINDOWS 7+
        ...
        'D3D9
        If Not WinSystem.IsWin10 Then 'Not assessed on Windows 10
        ...
        'D3D10
        If Not WinSystem.IsWin81 OrElse Not WinSystem.IsWin10 Then 'Not assessed on Windows 10
